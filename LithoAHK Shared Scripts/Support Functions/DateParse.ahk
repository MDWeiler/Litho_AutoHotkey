/*
	...
	About: License
		- Version 1.01 by Titan <https://ahknet.autohotkey.com/~Titan/#dateparse>.
		- Licenced under GNU GPL <http://creativecommons.org/licenses/GPL/2.0/>.

	SetFormat has been Depredated; function updated to use Format
...
DateParse(str) {
	...
	; Original Formating and Calculation -- SetFormat Depredated 
	; f = %A_FormatInteger%  -- SetFormat Depredated 
	; SetFormat, Float, 02.0 -- SetFormat Depredated 
	; d := (d3 ? (StrLen(d3) = 2 ? 20 : "") . d3 : A_YYYY)
	; 	. ((d2 := d2 + 0 ? d2 : (InStr(e2, SubStr(d2, 1, 3)) - 40) // 4 + 1.0) > 0 ? d2 + 0.0 : A_MM)
	; 	. ((d1 += 0.0) ? d1 : A_DD) . t1 + (t4 = "pm" ? 12.0 : 0.0) . t2 + 0.0 . t3 + 0.0
	; SetFormat, Float, %f% -- SetFormat Depredated
	d := (d3 ? (StrLen(d3) = 2 ? 20 : "") . d3 : A_YYYY)  ; Year
		. Format("{:02.0f}", ((d2 := d2 + 0 ? d2 : (InStr(e2, SubStr(d2, 1, 3)) - 40) // 4 + 1.0) > 0 ? d2 + 0.0 : A_MM))  ; Month
		. Format("{:02.0f}", ((d1 += 0.0) ? d1 : A_DD))   ; Day
		. Format("{:02.0f}", t1 + (t4 = "pm" ? 12.0 : 0.0))  ; Hour
		. Format("{:02.0f}", t2 + 0.0) ; Minute
		. Format("{:02.0f}", t3 + 0.0) ; Seconds
	Return, d
}